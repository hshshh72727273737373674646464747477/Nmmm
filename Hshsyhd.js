 ...
 * @param {string} outputFile - The file path to save generated code.
 */
function simulateNodeCodeGeneration(message, outputFile) {
...
`;
      ...
      // Save to file
      fs.writeFile(outputFile, generatedCode, 'utf8', (err) => {
        if (err) {
          console.error(`Error saving generated code to file: ${err.message}`);
        } else {
          console.log(`Generated code saved successfully to file: ${outputFile}`);
        }
        rl.close();
      });
    }
...
rl.question('Enter message to start AI code generator: ', (inputMessage) => {
  ...
  console.log(`\nConsole input message: "${trimmedMessage}"`);

  const outputFileName = 'generated-ai-code.js';
  const outputFilePath = path.resolve(__dirname, outputFileName);

  simulateNodeCodeGeneration(trimmedMessage, outputFilePath);
});
